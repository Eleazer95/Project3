...
def finish_withdraw():
    ...
    if float(withdraw_amount.get()) == 0:
        withdraw_notif.config(text="You cannot withdraw zero amount. Type in a Positive Value", fg="red")
        return
...
#Image imports
img = Image.open('bank2.jpg')

img = img.resize((150,150))
img = ImageTk.PhotoImage(img)

#labels
Label(master, text = "EAx Banking Services", font=('Calibri', 14)).grid(row=0,sticky=N,pady=10)
Label(master, text = "Trusted and Reliable Banking Services", font=('Calibri', 12)).grid(row=1,sticky=N)
